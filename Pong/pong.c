...
#include <locale.h> // para la estrella
...
void draw_paddle(Paddle *paddle) {
    for (int i = 0; i < paddle->height; i++) {
        mvaddch(paddle->y + i, paddle->x, paddle->display);
    }
}

void draw_ball(Ball *ball) {
    mvaddch(ball->y, ball->x, ball->display);
}

void move_paddle(Paddle *paddle, int dir, int up_key, int down_key) {
    if (dir == up_key && paddle->y > 0) {
        paddle->y--;
...
void draw_score(int score1, int score2) {
    ...
    char asterisks[COLS + 1];       // +1 para el carácter nulo de final de cadena
    memset(asterisks, '*', COLS);   // Para llenar la fila de '*'
    asterisks[COLS] = '\0';         // Agrega el carácter nulo al final de la cadena
    mvprintw(3, 0, "%s", asterisks);
    if (score1 > score2) {
        mvprintw(1, COLS / 2 - 7, "* ★       *");
...
int main() {
    ...
    setlocale(LC_ALL, "");  // caracteres especiales (estrella)
    initscr();              // ncurses
